...
class AveragePrecisionCallback(vision.Callback):
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        last_output = last_output.detach().cpu()
        last_target = last_target.detach().cpu()
        self.ap_meter.add(last_output, last_target)
    
    def on_epoch_end(self, last_metrics, **kwargs):
        mAP = 100 * self.ap_meter.value().mean()
        loss = self.ap_meter.value()[0]
        OP, OR, OF1, CP, CR, CF1 = self.ap_meter.overall()
        OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k = self.ap_meter.overall_topk(3)
        print(f'Test: \t mAP {mAP:.3f}')
        print(f'OP: {OP:.4f}\t'
              f'OR: {OR:.4f}\t'
              f'OF1: {OF1:.4f}\t'
              f'CP: {CP:.4f}\t'
              ...
              f'CF1_3: {CF1_k:.4f}')
        print('----------')
        return {'last_metrics': last_metrics + [mAP]} # for display
